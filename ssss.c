...
char** split_with_args(const char* secret, int threshold, int num_shares, int use_hex, const char* token, int quiet, 
                      int* num_output_shares, int security_bits, int use_diffusion)
{
  unsigned int fmt_len;
  mpz_t x, y, coeff[threshold];
  int deg, i;
  char share_buf[MAXLINELEN];
  *num_output_shares = 0;

  char** output_shares = malloc(num_shares * sizeof(char*));
  if (!output_shares) {
    fatal("Failed to allocate memory for shares");
    return NULL;
  }

  for(fmt_len = 1, i = num_shares; i >= 10; i /= 10, fmt_len++);
  if (!quiet) {
    printf("Generating shares using a (%d,%d) scheme with ", 
           threshold, num_shares);
    if (security_bits)
      printf("a %d bit", security_bits);
    else
      printf("dynamic");
    printf(" security level.\n");
  }

  // Calculate security bits if not provided
  if (!security_bits) {
    security_bits = use_hex ? 4 * ((strlen(secret) + 1) & ~1): 8 * strlen(secret);
    if (!field_size_valid(security_bits))
      fatal("security level invalid (secret too long?)");
    if (!quiet)
      fprintf(stderr, "Using a %d bit security level.\n", security_bits);
  }

  field_init(security_bits);

  mpz_init(coeff[0]);
  field_import(coeff[0], secret, use_hex);

  // Apply diffusion only if explicitly requested
  if (use_diffusion) {
    if (degree >= 64)
  ...
  for(i = 0; i < num_shares; i++) {
    ...
    share_buf[0] = '\0';
    if (token)
      sprintf(share_buf, "%s-", token);
    sprintf(share_buf + strlen(share_buf), "%0*d-", fmt_len, i + 1);
    
    FILE* memstream = open_memstream(&output_shares[i], &(size_t){0});
    fputs(share_buf, memstream);
    field_print(memstream, y, 1);
    fclose(memstream);
  ...
  *error = 0;
  ...
  for (i = 0; i < num_shares; i++) {
    ...
    *a++ = 0;
    ...
    mpz_set_ui(x, j);
    mpz_init_set_ui(A[num_shares - 1][i], 1);
    for(j = num_shares - 2; j >= 0; j--) {
      mpz_init(A[j][i]);
  ...
  if (restore_secret(num_shares, A, y)) {
    *error = 1;
    for (i = 0; i < num_shares; i++) {
  ...
  field_print(memstream, y[num_shares - 1], use_hex);
  fclose(memstream);
